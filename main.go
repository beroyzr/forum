...
func createPostHandler(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodPost {
			...
			userID := 1 // Bu örnekte sabit bir kullanıcı ID'si kullanıyoruz
...
func commentHandler(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodPost {
			postID := r.FormValue("post_id")
			content := r.FormValue("comment") // Değişiklik burada
			userID := 1                       // Bu örnekte sabit bir kullanıcı ID'si kullanıyoruz
...
func guestLoginHandler(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodGet {
			// db'yi kullanarak basit bir işlem yapabilirsiniz, örneğin boş bir sorgu çalıştırmak gibi.
			_ = db.Ping() // Bu, veri tabanına bağlı olup olmadığınızı kontrol eder.
...
// Kullanıcı oturumunu kontrol eden middleware
